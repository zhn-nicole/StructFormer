...
def get_lndm(path_img, path_out, start_id=0, dlib_path=""):
    ...
    for fld in folder_list[:]:
        imglist_all = [f[:-4] for f in listdir(join(path_img, fld)) if
                       isfile(join(path_img, fld, f)) and f[-4:] == ".jpg"]
                ...
                if land_mask:
                    ...
                    def draw_line(offset, pt_st, pt_end):
                        cv2.line(img_lndm, (int((landmarks.part(offset + pt_st).x - t_x) * ratio_w),
                                 ...
                                 int((landmarks.part(offset + pt_end).y - t_y) * ratio_h)), (0, 0, 255), line_px)
                    ...
                    contours = np.concatenate((contours, np.array(
                        [[(landmarks.part(0).x - t_x) * ratio_w, (landmarks.part(19).y - t_y) * ratio_h]])), axis=0)
                    for p in range(17):
                        contours = np.concatenate((contours, np.array(
                            [[(landmarks.part(p).x - t_x) * ratio_w, (landmarks.part(p).y - t_y) * ratio_h]])), axis=0)
                    contours = np.concatenate((contours, np.array(
                        [[(landmarks.part(16).x - t_x) * ratio_w, (landmarks.part(24).y - t_y) * ratio_h]])), axis=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', type=str, help='directory with input data', default='/home/user/project/structformer/dataset/celeba/clr')
    parser.add_argument('--output', type=str, help='directory for output', default='/home/user/project/structformer/dataset/celeba_new/')
