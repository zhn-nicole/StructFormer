...
def set_comp_device(FLAG_GPU): #设备设置函数
...
def load_model(model_dir, model_name, arch_name, device_comp, arch_type, DATA_PARAMS, epoch_start = 0, label_num=1200):
    arch = importlib.import_module('arch.arch_' + str(arch_type))

    model = getattr(arch, arch_name)(num_classes=label_num, encode_one_hot = True)
    if arch_name=='Generator':
        model.apply(weights_init)

    if epoch_start > 0:
        model.load_state_dict(torch.load(model_dir + model_name + '_ep' + str(epoch_start) + '.pt h'))
